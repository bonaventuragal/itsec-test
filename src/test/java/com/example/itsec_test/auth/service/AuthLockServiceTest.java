...
class AuthLockServiceTest {
    ...
    void testOnLoginFailureFirstTime() {
        ...
        verify(keyValueStoreProvider).saveValue(eq("login_fail_count:user1"), eq(1),
                eq(Duration.ofMinutes(authLockService.LOGIN_FAIL_COUNT_EXPIRY_MINUTES)));
    ...
    void testOnLoginFailureIncrement() {
        ...
        authLockService.onLoginFailure("user1");
        verify(keyValueStoreProvider).saveValue(eq("login_fail_count:user1"), eq(3),
                eq(Duration.ofMinutes(authLockService.LOGIN_FAIL_COUNT_EXPIRY_MINUTES)));
        verify(keyValueStoreProvider, never()).saveValue(eq("login_locked:user1"), eq(true), any());
    }
    ...
    void testOnLoginFailureLockTriggered() {
        ...
        authLockService.onLoginFailure("user1");
        verify(keyValueStoreProvider).saveValue(eq("login_fail_count:user1"), eq(5),
                eq(Duration.ofMinutes(authLockService.LOGIN_FAIL_COUNT_EXPIRY_MINUTES)));
        verify(keyValueStoreProvider).saveValue(eq("login_locked:user1"), eq(true),
                eq(Duration.ofMinutes(authLockService.LOGIN_LOCK_DURATION_MINUTES)));
    }
}
